...
class basicsActivity : ComponentActivity() { //Por si hay confusion, la clase se debe llamar igual que el archivo
...
private fun mainFunction() { //Funcion principal
    LazyColumn( //LazyColumn (Reemplazo de Recycler view o Scroll View)
        ...
        item {//Se debe de crear un item para las lazycolumns, dentro del item va to-do lo que va a contener esta lazy column
            ...
            Text(text = "Texto de prueba de parrafo", color = Color.White) //Otro texto con color blanco
            Text(text = "Segundo texto de prueba", color = Color.White) //Otro texto con color blanco
            LazyRow( //LazyRow permite una fila (horizontal) fuera de la aplicacion, pudiendo deslizar a la izquierda o derecha dependiendo
                ...
                modifier = Modifier.fillMaxWidth().padding(16.dp) //Que ocupe el maximo de ancho, pero con separacion de 16dp a la izquierda y derecha
            ) {
                item{//Item, ya que los objetos tipo "Lazy" necesitan un item
                    Text(text = "Hola soy Yo", color = Color.White) //Textos para comprobar que funcione bien
